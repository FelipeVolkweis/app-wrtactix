...
Q_LOGGING_CATEGORY(FREEANGLES, "FreeAngles");

QList<FreeAngles::Interval> FreeAngles::getFreeAngles(const Position &watcher, const Position &initialPos,
                                                      const Position &finalPos, const QList<Obstacle> &obstacles,
                                                      bool returnObstructed) {
...
QList<FreeAngles::Interval> FreeAngles::getFreeAngles(const Position &watcher, float initialPosAngle,
                                                      float finalPosAngle, const QList<Obstacle> &obstacles,
                                                      bool returnObstructed) {
    return _getFreeAngles(watcher, initialPosAngle, finalPosAngle, obstacles, returnObstructed);
}

QList<FreeAngles::Interval> FreeAngles::_getFreeAngles(const Position &watcher, float initialPosAngle,
                                                       float finalPosAngle, const QList<Obstacle> &obstacles,
                                                       bool returnObstructed) {
    ...
    if (initialPosAngle > finalPosAngle) {
        if (finalPosAngle == 0) {
            ...
            freeAngles.insert(finalPosAngle, 0);
            freeAngles.insert(initialPosAngle, 0);
            freeAngles.insert(2 * GEARSystem::Angle::pi, 0);
        }
    ...
    for (int i = 0; i < tmpObstacles.size(); i++) {
        ...
        //        std::cout << "obstacle #" << i << ", initial=" << GEARSystem::Angle::toDegrees(obst.initialAngle()) <<
        //        ", final=" << GEARSystem::Angle::toDegrees(obst.finalAngle()) << "\n";
        ...
        if (size % 2 == 0) {
            for (int fa = 0; fa < size; fa += 2) {
                ...
                //                std::cout << "testing with interval, angI=" << GEARSystem::Angle::toDegrees(angI) <<
                //                ", angF=" << GEARSystem::Angle::toDegrees(angF) << "\n";
                ...
                } else if (obst.initialAngle() > angI && obst.initialAngle() < angF && obst.finalAngle() > angI &&
                           obst.finalAngle() < angF) {
                    //                    std::cout << "caso 3\n";
    ...
    //         if((curr.angFinal()==next.angInitial() || (curr.angFinal()-GEARSystem::Angle::twoPi==next.angInitial()))
    //         && curr.obstructed()==next.obstructed()) {
    //             retn.removeAt(i);
    //             retn.removeAt(nextI);
